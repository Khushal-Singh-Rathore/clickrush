...
export const Game: React.FC = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] flex items-center justify-center px-6 py-10">
      <div className="w-full max-w-2xl p-8 md:p-12 rounded-3xl bg-[#0f2413]/40 backdrop-blur-xl border border-white/20 shadow-2xl text-center space-y-8 relative">
        ...
        {gameState === 'ACTIVE' && !isGameCompleted && (
          <div className="space-y-8">
            {/* Top HUD bar */}
            <div className="flex items-center justify-between p-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20">
              <div className="flex items-center gap-2 text-white">
                <Timer className="w-5 h-5 animate-pulse text-amber-300" />
            ...
            <div className="py-6 flex flex-col items-center justify-center space-y-6">
              <button
                ...
                className={`w-56 h-56 rounded-full bg-gradient-to-b from-white/90 to-white/60 text-[#0f2112] shadow-2xl hover:scale-105 transition-all duration-75 flex flex-col items-center justify-center gap-2 cursor-pointer select-none active:scale-95 border-4 border-white/40 ${
                  isClicking ? 'scale-95 ring-8 ring-white/30' : 'scale-100'
                }`}
              >
                <Zap className={`w-12 h-12 fill-current transition-transform ${isClicking ? 'scale-125' : 'scale-100'}`} />
                <span className="font-heading font-bold text-2xl tracking-wider">CLICK!</span>
              </button>

              <p className="text-xs text-white/80 tracking-wide uppercase font-semibold">
        ...
        {gameState === 'CONNECTING' && !isGameCompleted && (
          <div className="py-16 space-y-4">
            ...
            <h3 className="font-heading text-xl font-bold text-white">Connecting to Server...</h3>
        ...
        {isGameCompleted && (
          <div className="py-6 space-y-6 animate-fade-in">
            <div className="w-16 h-16 rounded-full bg-white/20 text-amber-300 flex items-center justify-center mx-auto border border-white/30 shadow-inner">
              <Trophy className="w-8 h-8 fill-current" />
            </div>

            <div className="space-y-1">
              <h2 className="text-3xl font-bold text-white">Game Completed!</h2>
              <p className="text-sm text-white/80">Your score has been verified and saved to the server.</p>
            </div>

            {/* Summary Score Card */}
            <div className="p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 grid grid-cols-2 gap-4 max-w-md mx-auto">
              <div className="space-y-1">
                <span className="text-xs uppercase font-semibold text-white/70">Total Clicks</span>
                <p className="text-3xl font-bold text-white">{displayedResult?.click_count ?? clickCount}</p>
              </div>
              <div className="space-y-1">
                <span className="text-xs uppercase font-semibold text-white/70">Final Score</span>
                <p className="text-3xl font-bold text-amber-300">{displayedResult?.score ?? clickCount}</p>
              </div>
            </div>
        ...
        {!isGameCompleted && (gameState === 'ERROR' || gameState === 'DISCONNECTED') && (
          <div className="py-12 space-y-6">
            <div className="w-14 h-14 rounded-full bg-red-500/20 text-red-300 flex items-center justify-center mx-auto border border-red-400/40">
